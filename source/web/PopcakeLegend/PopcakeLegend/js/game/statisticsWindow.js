...
StatisticsWindow.prototype.Load = function () {
   ...
   // this.image.Load("images/pop_ups/Popup-window.png");
	...
	//Added Nov 17, 2015 
	//if range is more than 10 levels, only displays 10 items
	if( g_gameData.max_level - (this.level_base) >= 10 ){		
		this.upto_level = (this.level_base*10) + (10-1); 
	}else{
...
StatisticsWindow.prototype.GetFriendsAverage = function()
{
	...
	for (var i = this.level_base; i <= 10/*this.upto_level*/; i++) {
		var average_score = 0;
		var average_stars = 0;
		var average_error = 0;
...
StatisticsWindow.prototype.ShowGraph = function (gfx)
{
    ...
    if (this.radioGroup.index == 0) {
        ...
        this.graphObject.gy_margin_value = 4000;
        this.graphObject.gy_count = 5;
    ...
    if (you_values != 0) {
        //chealsea
        var baseIdx = g_gameData.curr_level_idx - this.level_base;
        var value = you_values[baseIdx];
		 if( value > 0) {
		     var coord = this.graphObject.DetectCurrentLevel(gfx, you_values, baseIdx);
    ...
    if (this.radioGroup.index == 0 || this.radioGroup.index == 2) {
        //Draw Legend

        gfx.DrawRect(760, 235, 100, 60, "rgb(255,255,255)", 2);
        gfx.FillRect(770, 250, 10, 10, "rgb(233,38,42)");
        gfx.FillRect(770, 270, 10, 10, "rgb(39,170,226)");
        ...
        gfx.DrawText("FRIENDS",
          788, 280,
           "rgb(255,255,255)",
           "13pt DJBCHALKITUP");
    } else {
        gfx.DrawRect(760, 235, 100, 40, "rgb(255,255,255)", 2);
        gfx.FillRect(770, 250, 10, 10, "rgb(39,170,226)");
...
GraphObject.prototype.DrawValue = function (gfx, values, text, color)
{
    ...
    **********************************************/
    ...
    for (var i = 0; i < values.length; i++) {
		...
		last_good_x = current_xcoord;
		last_good_y = current_ycorrd;
	...
	if(circle_list.length > 0 ){
		/*gfx.DrawText(text,
              last_good_x - this.gx_xcoord_distance + 10,
              last_good_y,
              color,
              "10pt Androgyne_TB");*/
	}
...
GraphObject.prototype.DetectCurrentLevel = function (gfx, values,index)
{
    var factor = /*Math.floor*/(values[index] / this.gy_margin_value) * this.gy_ycoord_distance;

    var current_ycorrd = this.baseY - factor;

    var targetX = this.baseX + ((index + 1) * this.gx_xcoord_distance);
