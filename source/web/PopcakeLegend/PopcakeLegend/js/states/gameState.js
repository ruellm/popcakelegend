...
var GAME_STATE_CORRECTION_ROTATE = 34;
var GAME_STATE_SHAPE_QUESTION = 35;
var GAME_STATE_INVERTED_GAME_TIMER = 36;
var GAME_STATE_MOVING_LINE = 37;
var GAME_STATE_DELAY_WINNER = 38;
...
var inorder_repeat_correctIDX = -1;
...
GameState.prototype.Load = function () {
    ...
    this.started = false;
    this.current_pre_process = 0;
    this.inorder_repeat_markIdx = -1;
    inorder_repeat_correctIDX = -1;
   ...
   // this.targetScore = 0;
    ...
    if (g_gameData.life > 0) {
        if (this.currentLevel.tutorial) {
            //zen
            if (g_gameMode == GAME_MODE_DOU) 
                this.objectiveWindow = new Tutorial_DUO_Window;
            else
                this.objectiveWindow = new Tutorial_SOLO_Window;
        ...
        this.objectiveWindow.fnPlay = function () {
            ...
            context.timerLevel = new Date().getTime();
        }
    } else {
...
//    this.timerLevel = new Date().getTime();
	...
	this.logo._image.src = ("images/ingame/small-logo-3fps.png");
	this.logo.Set(6, 3.0, true);
	this.logo._frameWidth = 1170 / 6;

	this.glowShapes = null;
	this.has_shapes = false;
	this.shape_major = -1;
	this.shape_count = 0;
	this.shapes_buttons = new Array();
}
...
*/
            this.baker._X = 160;
            this.baker._Y = 563;
            this.baker._width = 174;
            this.baker._height = 171;
...
*/
            ...
            this.baker_2._X = 160;
            this.baker_2._Y = 563;
            this.baker_2._width = 174;
            this.baker_2._height = 171;
            this.baker_2._visible = false;


			"images/themes/sample/small-logo-3fps.png"
    ...
    this.yesBtn._fnMouseDownEvnt = function () {	
        ...
        switch (g_MonetizeMode) {
            ...
            case 4:
                ...
                if (context.currentLevel.inverted) return;
    ...
    */
    ...
    this.noBtn._fnMouseDownEvnt = function () {
        ...
        if (g_MonetizeMode == 5) {
            ...
            // FOr level 12 - again client is not thinking logical before
            // deciding on specifications, this code is full of hacks and
            // dirty codes, similar to client fickle mind

            //this is fail case but player reached more than 5 in a row
            if (g_gameData.curr_level_idx == 12) {
                
                // the current index is point to the next to guess
                var index_to_check = context.inorder_currentIdx-1; 
                if( index_to_check < context.inorder_repeat_markIdx)
                    index_to_check = context.inorder_repeat_markIdx;

                if(index_to_check >= 4){
                    inorder_repeat_correctIDX = index_to_check;                
                    context.DeclareStar_Level_12();
                    return;
                }
                else {
                    var message = ["Get 5 in a row to succeed!"];
                    context.ShowFailedPopup(message, "18pt Androgyne_TB");
                }
...
GameState.prototype.LoadBoardInfo = function (level) {
    ...
    // Cards hidden implementation
    var cards_hidden = new Array();
    if (this.currentLevel.cards_hidden) {
        while (cards_hidden.length < this.currentLevel.cards_hidden) {

            var random_number = Math.floor(Math.random() * 9);
            var found = false;
            for (var m = 0; m < cards_hidden.length; m++) {
                if (cards_hidden[m] == random_number) {
                    found = true;
                    break;
                }
            }
            ...
            if (!found) {
                cards_hidden.push(random_number);
            }
        }
    }
...
GameState.prototype.ShowNextSmallIcon = function () {
    ...
    if (currentIndexShow < this.currentChallenge.icons.length) {
        ...
        if (this.currentLevel.inverted) {
            
            this.state = GAME_STATE_INVERTED_GAME_TIMER;
...
GameState.prototype.StartChallenge = function ()
{
    this.state = GAME_STATE_PLAY;
...
GameState.prototype.Evaluate = function (index) {
    ...
    if (this.currentChallenge.inorder) {
        ...
        } else if (this.currentChallenge.inorder_repeat) {

			if( this.inorder_currentIdx > this.inorder_repeat_markIdx) {
				this.inorder_repeat_markIdx = this.inorder_currentIdx-1;
				if(this.inorder_repeat_markIdx <0 )
					this.inorder_repeat_markIdx = 0;
			}
            
			this.inorder_currentIdx = 0;
            this.correctIndex_list = new Array();

            // reset all correct_list
            for (var s = 0; s < this.correct_list.length; s++) {
                this.correct_list[s] = false;
            }

            // flip back the open cards            
            for (var i = 0; i < this.board.length; i++) {
                if (this.board[i].isOpen && i != index) {
    ...
    if (count == this.correct_list.length) {
        ...
        if (g_gameData.curr_level_idx==12) {
            //yeah special condition for level 12.. this is dirty style of code
            inorder_repeat_correctIDX = this.inorder_currentIdx;
            this.DeclareStar_Level_12();
            return;
        }

        if (this.currentLevel.id == 16) {
            this.state = GAME_STATE_DELAY_WINNER;
            this.timerDelayWinner = new Date().getTime();
            return;
        }
...
GameState.prototype.DoFinishEX_II = function ()
{
...
GameState.prototype.DoFinishEX_III = function ()
{
...
GameState.prototype.DeclareStar_Level_12 = function ()
{
    // This function exist because Client and Walldorf do not 
    // know how to simulate logical cases and condition before 
    // deciding on specifications, therefore resulting to Hack
    // and dirty code all over.
    // Declare winner section (per challenge)
    // this is a duplicate function from Evaluate()
    // this function needs to be clean-up
    this.state = GAME_STATE_STARIN;
    this.ProduceStar(1);
    this.success = true;
    ...
    var msgidx = 3;
    if (this.inorder_repeat_markIdx < 5) {
        //threshold
        msgidx = 2;
    }

    var idex = Math.floor(Math.random() * message[msgidx].length);
    g_message = message[msgidx][idex];

    /*var m = this.inorder_repeat_markIdx;
    if (m == -1) {
        m = this.inorder_currentIdx;
    } else if (this.inorder_currentIdx > this.inorder_repeat_markIdx) {
        m = this.inorder_currentIdx;
    }*/

    g_gameData.total_accumulated_stars += ((inorder_repeat_correctIDX + 1) * 3);
...
GameState.prototype.ShowNextStar = function () {
    ...
    if (starNextShow < this.star_list.length) {
        ...
        this.PRE_FinishLevel();
    }
}

GameState.prototype.PRE_FinishLevel = function ()
{
    //////////////////////////////////////////////////////////////
    // Levels with shape guessing - Milestone 6 Ex: level 13
    //////////////////////////////////////////////////////////////
    if (this.has_shapes && this.state != GAME_STATE_SHAPE_QUESTION &&
        g_gameData.curr_challenge_idx + 1 >= this.currentLevel.challenges_list.length) {
        this.state = GAME_STATE_SHAPE_QUESTION;
        for (var i = 0; i < this.shapes_buttons.length; i++) {
            this.shapes_buttons[i]._visible = true;
        }
        return;
    } else {
...
GameState.prototype.FinishGameSuccess = function ()
{
...
GameState.prototype.Watch3Seconds = function (count) {
    ...
    if (this.currentLevel.inverted) {
        ...
        for (var i = 0; i < this.small_icon_list.length; i++) {
            if (!this.small_icon_list[i].isOpen) {
                this.small_icon_list[i].Flip();
                this.small_icon_list[i].fnAnimateComplete = null;
            }
        }
...
GameState.prototype.IsBoardOpened = function ()
{
    for (var i = 0; i < this.board.length; i++) {
        if (this.board[i].isOpen)
            return true;
...
GameState.prototype.GenerateIconChallenge = function () {
    ...
    /////////////////////////////////////////////////////
    // Pre process
    if (this.currentLevel.pre_process && 
        !this.IsPreProcessDone()) {
        preprocess_state = true;
        this.ProcessNextPRE_PROCESS();
        return false;
    }
    /////////////////////////////////////////////////////
    ...
    var length_side = length;
    if (length > 5)
        length_side = 5;

    var boardw = ((SMALL_ICONBOX_WIDTH + 5) * length_side);
    ...
    this.glowShapes = null;
    if (this.currentChallenge.shape_type != -1) {
        var shapes = [
        ...
        this.glowShapes= new GlowImage();
        this.glowShapes.Load(shapes[this.currentChallenge.shape_type], 60);

        if (1/*this.has_shapes == false*/) {
            //Perform first time initializations in here
            this.has_shapes = true;
            this.shape_major = Math.floor(Math.random() * 3);
            ...
            this.current_Shape = new ImageObject();
            this.current_Shape.Load(shapes2[this.currentChallenge.shape_type]);

            this.shape_img = new ImageObject();
            this.shape_img.Load(shapes2[this.shape_major]);
            ...
            var button_width = 40;
            var boardw = ((button_width + 8) * 5);
            var sourcex = (DEFAULT_WINDOW_WIDTH / 2) - (boardw / 2);

            var x = sourcex;
            while (s < 6) {
                var button = new RoundButtonText;
                button.Create(button_width, 40,
                    "rgb(241,104,34)",
                    "rgb(255,255,255)",
                    "rgb(255,255,255)",
                    "rgb(255,0,0)",
                ...
                button._fnMouseDownEvnt = function () {
                    ...
                    if (this.data == context.shape_count) {
                        context.FinishGameSuccess();
                    } else {
                        var message = ["You answered wrong, the correct answer is " +
                            context.shape_count];

                        context.ShowFailedPopup(message, "15pt Androgyne_TB");
                    }
                };
                this.shapes_buttons.push(button);
                this._uiManagerLocal.Add(button);
                s++;
                x += ((button_width + 8));
            }
        ...
        if (this.currentChallenge.shape_type == this.shape_major) {
            this.shape_count++;
        }
    }
...
GameState.prototype.IsPreProcessDone = function ()
{
    return this.current_pre_process >= this.currentLevel.pre_process.length;
}

GameState.prototype.ProcessNextPRE_PROCESS = function ()
{
    switch (this.currentLevel.pre_process[this.current_pre_process++])
    {
        case 0:
            this.DoRotate();
            return;
...
GameState.prototype.DoMoveLine = function ()
{
    if (this.currentChallenge.move_line == null) return;
    
    var callback_set = false;
    var context = this;
    for (var k = 0; k < 3; k++){
        var move_type = this.currentChallenge.move_line[k];
        var SHIFT_VALUE = 0;
        if (move_type == 1)
            SHIFT_VALUE = -(ICONBOX_WIDTH+20);
        else if (move_type == 2)
            SHIFT_VALUE = (ICONBOX_WIDTH+20);
        else
            continue;
        ...
        var index = k * 3;
        for (var h = 0; h < 3; h++) {
            this.board[index].GoTo(
                this.board[index]._X + SHIFT_VALUE,
                this.board[index]._Y);

            if (!callback_set) {
                this.board[index].fnAnimateComplete = function () {
                    context.GenerateIconChallenge();
                };
                callback_set = true;
            }
            index++;
    ...
    this.state = GAME_STATE_MOVING_LINE;
}


var preprocess_state = false;
var cards_counter_for_close = 0;
var open_cards = 0;
var hack1_flag = false;

GameState.prototype.BeForePlay = function () {

    var context = this;
    cards_counter_for_close = 0;
    open_cards = 0;
    hack1_flag = false;

    // close all the cards
    for (var i = 0; i < this.board.length; i++) {
        if (!this.board[i].isOpen) continue;
        this.board[i].Flip();
        this.board[i].done = false;
        open_cards++;

        this.board[i].fnAnimateComplete = function () {
            cards_counter_for_close++;
            this.fnAnimateComplete = null;
            if (cards_counter_for_close >= open_cards && !hack1_flag) {
                hack1_flag = true;
                context.GenerateIconChallenge();
...
GameState.prototype.SetPopupCompleteScore = function ()
{
    var total = 0;
...
GameState.prototype.ShowFailedPopup = function (msg, style)
{
    var context = this;
	...
	if(style){
		this.popupComplete.msg_style = style;
	} 
    ...
    switch (this.state) {
        ...
        case GAME_STATE_ROTATE_1_EXROTATE: {
            ...
            if (diff >= 1.5) {
                ...
                ///////////////////////////////////////////////
                // Do the Swap
                // maintain a copy before rotation
                var type_copy = new Array();
                for (var i = 0; i < this.board.length; i++) {
                    type_copy.push(this.board[i].type);
                }

                if (this.currentChallenge.rotate_angle == 180) {
                    index_src = 8;
                    for (var i = 0; i < 9; i++) {
                        var type = type_copy[index_src--];
                        this.board[i].LoadFace(type);
                    }
                } else if (this.currentChallenge.rotate_angle == 90) {
                    var index_src = 0;
                     for (var d = 2; d >= 0; d--) {
                         for (var c = 0; c < 3; c++) {
            ...
            for (var i = 0; i < this.board.length; i++) {
                ...
                this.board[i].Rotate(0);
                this.state = GAME_STATE_CORRECTION_ROTATE;

                // if this is pre process and preprocess is rotate without showing
                // and there are still other pre-process that will happen, process next pre-process                
                if (preprocess_state && !this.IsPreProcessDone() &&
                    this.currentLevel.pre_process[this.current_pre_process - 1] == 0) {
                    // reset the board angle
                                  
                    if (i == 0) {
                        this.board[i].fnAnimateComplete = function () {
                           context.ProcessNextPRE_PROCESS();
                        };
                     }
                    ...
                    if (1) {
                        this.board[i].fnAnimateComplete = function () {
                            /*for (var jm = 0; jm < context.board.length; jm++) {
                                context.board[jm].Flip();
                                context.board[jm].fnAnimateComplete
                            }*/
                            //this.Flip();
                            this.fnAnimateComplete = null;
                            context.state = GAME_STATE_ROTATE_3_ROTATE_DONE;
    ...
    switch (this.state) {
        ...
        case GAME_STATE_SWITCH_3: {
            ...
            if (diff >= 2) { 
                ...
                if (!(preprocess_state && this.currentLevel.pre_process[this.current_pre_process - 1] == 1)) {
                    for (var i = 0; i < switch_index.length; i++) {
                        this.board[switch_index[i]].Flip();
                        this.board[switch_index[i]].fnAnimateComplete = null;
                    }
...
GameState.prototype.Update = function (elapsed) {
    ...
    if (!handled) {
        switch (this.state) {
            ...
            case GAME_STATE_DELAY_WINNER:
                {
                    var currTime = new Date().getTime();
                    var diff = (currTime - this.timerDelayWinner) / 1000;
                    if (diff >= 2) {
                        this.DoFinishEX_II();
                        this.DoFinishEX_III();
                    }
  ...
  /*  if (this.level_score < this.targetScore) {
    ...
    if (this.glowShapes) {
        this.glowShapes.Update(elapsed);
    }
}
...
GameState.prototype.DoRotate = function () {
   /* if (this.currentChallenge.rotate_angle == 90) {
        var type_copy = new Array();
        for (var i = 0; i < this.board.length; i++) {
            this.board[i].angle = this.currentChallenge.rotate_angle;
            type_copy.push(this.board[i].type);
    ...
    board_angle = 0;
    board_angle_target = this.currentChallenge.rotate_angle;
...
GameState.prototype.EvaluateGlow = function () {
    ...
    if (g_highlight_Shuffle || g_highlight_Watch3Seconds ||
		...
		// PBM has only single theme
		this.monetize_arrow.Load("images/ingame/arrow/SpriteBooster.png");
        this.monetize_arrow.Set(8, 30.0, true);
        this.monetize_arrow._frameWidth = 960/8;
	...
	if(g_giftHighlight) {
		...
		/*    switch (g_gameData.theme) {
		 ...
		 this.gift_arrow.Load("images/ingame/arrow/SpriteBooster.png");
		 this.gift_arrow.Set(8, 30.0, true);
		 this.gift_arrow._frameWidth = 960 / 8;
	}
}
GameState.prototype.ProceedNextChallenge = function () {
   
    if (!this.GenerateIconChallenge()) return;
...
GameState.prototype.Draw = function (gfx) {
    ...
    // Temporary Text
	var text = "Level " + (g_gameData.curr_level_idx + 1);
	
	var ctx = gfx._canvasBufferContext;
    var style = "28pt Calibri";
    ctx.font = style;
    var textWidth = ctx.measureText(text);
	var text_X = DEFAULT_WINDOW_WIDTH - (textWidth.width) - 10; 
	
	gfx.DrawText(text,
           text_X, 30, "rgb(255,255,255)", style);
   ...
   /* if (g_gameData.theme==THEME_TYPE_SAMPLE) {
    ...
    /////////////////////////////////////////////////////
    if (this.state >= GAME_STATE_ROTATE_1_EX && this.state <= GAME_STATE_ROTATE_2_ROTATE_DONE) {
    ...
    if (this.state == GAME_STATE_TIMER_VIEWING ||
        ...
        pct = count / 3;
        if (pct <= 0) pct = 0;
    } else if (this.state == GAME_STATE_INVERTED_GAME_TIMER) {
        ...
        if (diff > 3) {
            // this is a inverted level 
            for (var sc = 0; sc < this.small_icon_list.length; sc++) {
                this.small_icon_list[sc].Flip();
                this.small_icon_list[sc].fnAnimateComplete = null;
            }
    ...
    for (var sc = 0; sc < this.sideButtonList.length; sc++) {
        ...
        if (highlight) {
			...
			*/
		    ...
		    this.monetize_arrow._X = this.sideButtonList[sc]._X - 100;
            this.monetize_arrow._Y = this.sideButtonList[sc]._Y +10;
...
//    }
    ...
    if (this.glowShapes) {
        ...
        this.glowShapes._X = 214;
        this.glowShapes._Y = 319;
        this.glowShapes.Draw(gfx);

        this.current_Shape._X = this.glowShapes._X + 36;
        this.current_Shape._Y = this.glowShapes._Y + 36;
        this.current_Shape.Draw(gfx);
    }
    ...
    if (this.state >= GAME_STATE_BEFOREPLAY && this.state <= GAME_STATE_FLIPBACK
        ...
        this.state == GAME_STATE_INVERTED_GAME_TIMER ||
        this.state == GAME_STATE_DELAY_WINNER ||
        (this.state == GAME_STATE_WATCH3_SECONDS_0 && this.currentLevel.inverted)) {
	...
	//	} else {
          this.gift_arrow._X = 710;
		//}
		  this.gift_arrow._Y = 647;
    ...
    } else if (this.state == GAME_STATE_SHAPE_QUESTION) {
        this.DrawShapesQandA(gfx);
...
GameState.prototype.DrawWinnerState = function (gfx) {
    ...
    if (g_gameData.curr_level_idx == 12 && this.state != GAME_STATE_STARIN) {
        ...
        gfx.DrawText("x" + ((inorder_repeat_correctIDX+1) * 3),
        this.star_list[0]._X + 50, 
        this.star_list[0]._Y + 60,
          "rgb(242,105,37)", "12pt Androgyne_TB");
    }
}
...
GameState.prototype.DrawChallengeBoxContents = function (gfx) {
    ...
    if (this.small_icon_list) {
        ...
        for (var sc = 0; sc < this.small_icon_list.length; sc++) {
            ...
            if (this.currentChallenge.inorder &&
                this.currentChallenge.inorder_repeat &&
                this.inorder_repeat_markIdx != -1 &&
                this.inorder_repeat_markIdx == sc) {

                gfx.FillRect(
                    this.small_icon_list[sc]._X-2,
                    this.small_icon_list[sc]._Y-2,
                    this.small_icon_list[sc]._width+4,
                    this.small_icon_list[sc]._height+4,
                    "rgb(228,108,10)", 1);
            }
        ...
        if (this.currentChallenge.icons.length > 5) return;
...
GameState.prototype.DrawShapesQandA = function (gfx)
{
    ///...
    //draw the shape on top of buttons
    for (var i = 0; i < this.shapes_buttons.length; i++) {
        this.shapes_buttons[i].Draw(gfx);
    }

    gfx.DrawText("How many times ",
       400, 634, "rgb(255,255,255)", "18pt Calibri");

    gfx.DrawResized(this.shape_img._image, 570,
         610, 35, 35);

    gfx.DrawText("appear?",
      618, 634, "rgb(255,255,255)", "18pt Calibri");
}
    ...
    if (g_gameData.curr_challenge_idx + 1 < this.currentLevel.challenges_list.length) {
        ...
        for (var i = 0; i < this.board.length; i++) {
            ...
            if (this.board[i].isOpen) {
                ...
                if (this.currentChallenge.redo_level ||
                    this.currentLevel.inverted) {
                    this.currentLevel.Generate();
                    this.LoadBoardInfo(this.currentLevel);
                    this.current_pre_process = 0;
                    return;
                }else if (this.currentChallenge.switch_icon && !switchflag) {
                    switchflag = true;
                ...
                } else if (this.currentChallenge.move_line != null && !moveFlag) {
                    moveFlag = true;
                    this.board[i].fnAnimateComplete = function () {
                        context.DoMoveLine();
                        this.fnAnimateComplete = null;
        ...
        ////////////////////////////////////////////////////////  
        /*
		//-- Double access condition is disabled
		if (g_gameMode != GAME_MODE_DOU) {
        ...
        if (g_gameMode == GAME_MODE_SOLO) {
            ...
            if (bestData == null || this.level_score > (bestData.score + bestData.extra)) {
                if (g_gameMode != GAME_MODE_DOU) {
           ...
           if (chlsea/*bestData == null || this.popupComplete.total_score > (bestData.score + bestData.extra)*/) {
                if (1/*g_gameMode != GAME_MODE_DOU*/) {
                    // We dont store info for Game mode DOU
