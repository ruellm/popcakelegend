 ...
 <?php 
	...
	function reAuth($facebook)
	{
		...
		$params = array(
				...
				'redirect_uri' => 'https://apps.facebook.com/professorbrainmemory'
		);
	...
	if($DEBUG_FB_MODE==0){
		...
		if (!$fbuid) {
			...
			$user_profile = $facebook->api('/v2.4/me?fields=locale,id,name,currency');
			$name = $user_profile['name'];
			$locale = $user_profile['locale'];
			$currency = $user_profile['currency']['user_currency'];
		}
	}
	...
	echo "g_DBUserInfo.email ='". $userInfo->email . "';";
	...
	foreach ($friendsList["data"] as $value) {
		...
		if($result){		
			...
			foreach($rowarray as $row) {
				...
				//Retrie the name
				$query = "SELECT name from user where fbid=".$fbuid;		
				$result = $db->query($query);
				if($result){		
					$rowarray = $result->fetchall(PDO::FETCH_ASSOC);
					$row = $rowarray[0];
					echo "level.name ='" . $row['name'] . "';";
				}
...
<head>
    <title>Professor Brain Memory</title>
    ...
    <script src="js/game/mailinglistWindow.js"></script>
	<script src="js/game/tutorialWindow.js"></script>
...
<script>
	...
	var url = "https://apps.facebook.com/professorbrainmemory/";
	//var hostURL = "https://localhost/popcakelegend/"; // Temporary	
	
	/***********************************************************************/
	/************* CHANGE THIS WHEN Migrating server *******************/
	/**/  var hostURL = "https://professor-brain-memory.herokuapp.com/";   /**/
	/***********************************************************************/
	...
	function FBAcceess_Share(title, message)
	{
		...
		var picturePath = hostURL + "/images/app_icon.png";
		...
		FB.getLoginStatus(function(response) {
			if (response.status === 'connected') {
                ...
                {
                        method: 'feed',
                        name: "Professor Brain Memory",
                        link: 'https://apps.facebook.com/professorbrainmemory/',
                        picture: picturePath
                }, function (response) {
	...
	{
		// When changing the data, force SCRAPE it in here: https://developers.facebook.com/tools/debug/og/object/
		var coinsdef =[
			{coins: 50, url:  "http://professor-brain-memory.herokuapp.com/payment_system/coin_50.html"},
			{coins: 100, url: "http://professor-brain-memory.herokuapp.com/payment_system/coin_100.html"},
			{coins: 300, url: "http://professor-brain-memory.herokuapp.com/payment_system/coin_300.html"},
			{coins: 500, url: "http://professor-brain-memory.herokuapp.com/payment_system/coin_500.html"},
			{coins: 30, url:  "http://professor-brain-memory.herokuapp.com/payment_system/coin_30_.html"},
	...
	function FBAccess_GetEmail()
	{
		FB.api('/v2.3/me', { locale: 'en_US', fields: 'name, email' },
			function(response) {
					console.log(response.email);
					Ajax_SetEmail(response.email);
			}
		);		
	}
 ...
 <script type="text/javascript" src="//cpm1.affiz.net/tracking/ads_video.php" defer async></script>
    <script type="text/javascript">
        ...
        {          
            ...
            AFFIZVIDEO.init({
                     site_id: '315f315f32303838_ab3d78d175', 		// 	PBM actual/live
																	// For testing
                    //site_id: '315f315f32303839_9d50c5381d', 		// 	PBM video available always
                    //site_id: '315f315f32303930_f712c88c95', 		// 	PBM no video
	...
	<div class="modal hide" role="dialog">
	  ...
	  <div class="modal-footer"> 
		...
		<div style="width: 100%; text-align:center;"
			class="fb-like" 
			data-href="https://www.facebook.com/professorbrainmemory/" 
